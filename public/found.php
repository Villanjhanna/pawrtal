...
if ($search) {
    $where[]  = "(pet_type LIKE ? OR breed LIKE ? OR color LIKE ? OR found_place LIKE ? OR description LIKE ?)";
    $s = "%$search%";
...
$orderBy = match($sort) {
    ...
    'date_asc' => 'found_date ASC',
    'date_desc'=> 'found_date DESC',
    default    => 'created_at DESC',
};

$sql = "SELECT * FROM found_reports WHERE " . implode(' AND ', $where) . " ORDER BY $orderBy";
$stmt = $conn->prepare($sql);
if ($params) $stmt->bind_param($types, ...$params);
$stmt->execute();
$reports = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);

$totalActive = $conn->query("SELECT COUNT(*) AS c FROM found_reports WHERE status='active'")->fetch_assoc()['c'];
...
<title>Found Pets — Pawrtal</title>
...
.card-pill{position:absolute;top:10px;left:10px;font-size:10px;font-weight:600;padding:3px 9px;border-radius:99px;text-transform:uppercase;letter-spacing:.06em;background:rgba(45,106,79,.9);color:#fff;}
...
<nav>
  ...
  <div class="nav-links">
    ...
    <a href="found.php" class="active">Found pets</a>
  </div>
...
<div class="page-header">
  <div class="page-header-inner">
    <div class="page-header-top">
      <div class="page-title">Found pets</div>
    ...
    <form method="GET" class="filter-bar">
      <input type="text" name="search" class="filter-input filter-search"
             placeholder="Search by type, color, location…"
      ...
      <select name="sort" class="filter-input filter-select">
        ...
        <option value="date_desc" <?= $sort==='date_desc'?'selected':'' ?>>Found date (recent)</option>
        <option value="date_asc"  <?= $sort==='date_asc' ?'selected':'' ?>>Found date (oldest)</option>
      </select>
      <button type="submit" class="filter-btn"><i class="ti ti-search"></i> Search</button>
      <?php if ($search || $type || $gender || $sort !== 'newest'): ?>
        <a href="found.php" class="filter-clear"><i class="ti ti-x"></i> Clear</a>
      <?php endif; ?>
    </form>
  </div>
</div>
...
<div class="results-bar">
  <div class="results-count">
    Showing <strong><?= count($reports) ?></strong> found pet<?= count($reports) !== 1 ? 's' : '' ?>
    <?= $search ? ' matching <strong>'.htmlspecialchars($search).'</strong>' : '' ?>
  </div>
  <?php if (isset($_SESSION['user_id'])): ?>
  <a href="../dashboard/report_found.php" class="btn sm primary"><i class="ti ti-plus"></i> Report a found pet</a>
  <?php endif; ?>
</div>
...
<div class="grid-wrap">
  ...
  <div class="empty">
    <i class="ti ti-paw"></i>
    No found pet reports<?= $search ? ' matching "'.htmlspecialchars($search).'"' : '' ?>.<br>
    <?php if ($search || $type || $gender): ?>
      <a href="found.php" style="color:var(--green);font-weight:500;margin-top:8px;display:inline-block;">Clear filters</a>
    <?php endif; ?>
  </div>
  <?php else: ?>
    ...
    <a href="view_found.php?id=<?= $r['id'] ?>" class="report-card">
      <div class="card-img">
        ...
        <?php else: ?>
          <i class="ti ti-paw"></i>
        <?php endif; ?>
        <span class="card-pill">Found</span>
      </div>
      <div class="card-body">
        <div class="card-name">
